...
simgmm_dyn  = matrix(NA,nsims,12)
simgmm2_dyn  = matrix(NA,nsims,12)
simdid_dyn  = matrix(NA,nsims,12)

simgmm_overall  = matrix(NA,nsims,1)
simgmm2_overall  = matrix(NA,nsims,1)
...
if (simi > 2) {
  ...
  res <- data.frame(
    gmm_mean = colMeans(simgmm[1:simi,]),   # GMM means
    gmm2_mean = colMeans(simgmm2[1:simi,]),   # GMM means
    did_mean = colMeans(simdid[1:simi,]),   # DID means
    gmm_sd = apply(simgmm[1:simi,], 2, sd), # GMM standard deviations
    gmm2_sd = apply(simgmm2[1:simi,], 2, sd), # GMM standard deviations
  ...
  simgmm_dyn[simi,] <- agg.es.gmm$att.egt
  simgmm2_dyn[simi,] <- agg.es.id$att.egt
  simdid_dyn[simi,] <- agg.es.did$att.egt

  if (simi > 2) {
    # Compute column means and standard deviations
    res_dyn <- data.frame(
      gmm_mean = colMeans(simgmm_dyn[1:simi,]),   # GMM means
      gmm2_mean = colMeans(simgmm2_dyn[1:simi,]),   # GMM means
      did_mean = colMeans(simdid_dyn[1:simi,]),   # DID means
      gmm_sd = apply(simgmm_dyn[1:simi,], 2, sd), # GMM standard deviations
      gmm2_sd = apply(simgmm2_dyn[1:simi,], 2, sd), # GMM standard deviations
  ...
  simgmm_overall[simi,] <- agg.es.gmm$overall.att
  simgmm2_overall[simi,] <- agg.es.id$overall.att
  simdid_overall[simi,] <- agg.es.did$overall.att

  if (simi > 2) {
    # Compute column means and standard deviations
    res_overall <- data.frame(
      gmm_mean = mean(simgmm_overall[1:simi, ]),   # GMM means
      gmm2_mean = mean(simgmm2_overall[1:simi, ]),   # GMM means
      did_mean = mean(simdid_overall[1:simi, ]),   # DID means
      gmm_sd = sd(simgmm_overall[1:simi, ]), # GMM standard deviations
      gmm2_sd = sd(simgmm2_overall[1:simi, ]), # GMM standard deviations
