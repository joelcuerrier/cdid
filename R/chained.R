...
#' @param clustervars A vector of variables names to cluster on. At most, there can be two variables (otherwise will throw an error) and one of these must be the same as idname which allows for clustering at the individual level. By default, we cluster at individual level (when bstrap=TRUE).
#' @param est_method The method to compute group-time average treatment effetcs. The default is "chained". Later, dr, ipw and reg will be added. 
...
#Éventuellement utiliser ceci. Quelques modifications à faire avant.
# library(did)
